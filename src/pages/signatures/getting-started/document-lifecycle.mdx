...
subtitle: Learn how Idura Signatures handles your documents securely and safely.
---
...
You can download your document from the Idura Signatures API at any point **up until after `closeSignatureOrder` is called**.
...
If you wish to extend document lifetime so that it exists on Idura's servers beyond `closeSignatureOrder`, you can send `{retainDocumentsForDays: [number 1-7]}` as input to `closeSignatureOrder`.
The documents will then stay available on the signature order until the configured amount of days has passed. After that, they will be deleted from Idura's servers.
...
<GraphQLExample
  example={{
    ...
    variables: data => ({
      ...
      input: {
        ...closeSignatureOrderExample.variables(data).input,
