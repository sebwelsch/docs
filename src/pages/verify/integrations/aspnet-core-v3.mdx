...
subtitle: Accept MitID, Swedish BankID, Norwegian BankID and other eID logins with ASP.NET Core 3.1 and Idura Verify
---
...
1. [Register your application in Idura Verify](#register-your-application-in-idura-verify)
2. [Configure OAuth2 Code Flow](#configure-the-oauth2-code-flow)
3. [Configure your application to use Idura Verify](#configure-your-application-to-use-idura-verify)
...
To modify your existing application to work with Idura Verify, follow the steps below.
...
Once your application is registered, the dashboard will contain the necessary details to configure your application to communicate with Idura Verify. You will need the following information from the **General** tab of your application settings:

- **Client ID** to identify you application to Idura Verify.
- **Domain](/verify/getting-started/basics/#domains)** on which you will be communicating with Idura Verify.
...
The OIDC middleware requests both the `openid` and `profile` scopes by default, but note that Idura Verify by nature returns only the information derived from the underlying eID service. Changing the scopes does not affect the amount and nature of information delivered from the user information endpoint.
...
public void ConfigureServices(IServiceCollection services)
{
    ...
    .AddOpenIdConnect(options => {
        options.ClientId = Configuration["Idura:ClientId"]; // ClientID from application registration
        options.ClientSecret = Configuration["Idura:ClientSecret"]; // Client secret from oauth2 code flow
        options.Authority = $"https://{Configuration["Idura:Domain"]}/"; // Domain from application registration
...
4. The user's browser is redirected to the Idura Verify service, where the actual login takes place.
5. A callback with an issued _authorization code_ is sent back to the application and intercepted by the OIDC middleware.
6. The middleware calls the Idura Verify service to exchange the code for an _access token_. Note that this is a direct server-to-server call which, unlike the other communication, does not pass through the browser.
