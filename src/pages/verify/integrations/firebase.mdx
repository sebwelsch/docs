...
1. Login to your Idura Verify account - [https://dashboard.criipto.com/applications/add](https://dashboard.criipto.com/applications/add)
...
Once you register your login application at Idura Verify, you will have access to the following information necessary for leveraging Idura as an OpenID Connect provider in Firebase Authentication with Identity Platform:

- **Client ID** to identify your Idura tenant to Firebase. In the case below we chose `urn:criipto:samples:no1`
...
5. Enter a provider name (in this example we used "Idura").
6. Enter your Idura Application client id in the "Client ID" field.
7. Enter `https://{{YOUR_IDURA_DOMAIN}}` in the "Issuer (URL)" field.
...
9. You will now configure the OIDC integration. Copy the Firebase Callback URL and add it to the "Callback URL" field in your Idura Dashboard.
   ![Callback URL in Idura Dashboard](./firebase/criipto-dashboard-callback-url.png)
...
[In `@criipto/verify-react`](/verify/integrations/react/), you can use the `id_token` property of the `result` object to retrieve the ID token after the user signed in with Idura Verify.

```jsx
import { CriiptoVerify } from '@criipto/verify-react';
