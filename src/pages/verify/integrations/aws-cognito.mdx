...
1. Log in to your Idura Verify account - [https://dashboard.criipto.com/applications/add](https://dashboard.criipto.com/applications/add)
...
4. Enter a provider name (in this example we used "Idura")
5. Enter your Idura Application client id in the "Client ID" field
6. Enter your Idura Application client secret in the "Client Secret" field
7. Enter "openid" in "Authorize scope" field
8. Enter `https://{{YOUR_IDURA_DOMAIN}}` in the "Issuer" field
...
3. Mark "Idura" (or whatever provider name you picked for your AWS Cognito provider)
