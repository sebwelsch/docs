...
1. Login to you Idura Verify account - [https://dashboard.criipto.com/applications/add](https://dashboard.criipto.com/applications/add)
...
- Select the relevant domain from the `Available on domain` section. This will be the domain used to communicate to OneLogin. If you only subscribe to one Idura Verify domain, only one will be listed.
- Add a Client ID/Realm to identify the OneLogin Tenant in Idura Verify. In this example `urn:onelogin:criipto` has been assigned.
...
4. Click `Save`
5. The client secret will be displayed - This will be needed when configuring the Trusted IDP within OneLogin. Please note this is the only time the client secret will be displayed. Idura only stores the client secret as a hashed value, meaning it can not be retrieved again once generated and stored.
...
- To review your domain in Idura Veirfy navigate to Domains from the admin console and be sure to prefix `https://`
