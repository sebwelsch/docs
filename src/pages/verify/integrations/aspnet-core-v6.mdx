...
After you signed up for Criipto Verify, you must [register an application](https://dashboard.criipto.com/applications/add) before you can actually try logging in with any e-ID.
...
The OIDC middleware requests both the `openid` and `profile` scopes by default, you may configure additional scopes if your application is [configured with dyanamic scopes](https://docs.criipto.com/verify/getting-started/oidc-intro/#the-scope-parameter).
