...
This guide shows you how to use Idura Verify in a React single page application. Read the [full documentation](https://github.com/criipto/criipto-verify-react) or [download a sample application from GitHub](https://github.com/criipto/criipto-verify-react/tree/master/example).

## App switch support

`@criipto/verify-react` supports app switching for Danish MitID by a best-effort mobile-os detection and setting the relevant Idura Verify login hints.
...
Set up the Idura Verify SDK by wrapping your application in `CriiptoVerifyProvider`:
...
The library will also attempt to retrieve a user token on page load via SSO (if your Idura domain has SSO enabled).

You may wish to increase the "Token lifetime" setting of your Idura Application.
