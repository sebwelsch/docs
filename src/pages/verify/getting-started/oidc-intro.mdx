...
You must explicitly enable this feature [per application](https://manage.criipto.id/applications). 
...
We chose this approach as it is most often possible to send them through intermediaries if your architecture has such components (other OpenID Providers, for example).
...
For DK MitId specifically, you may also send a base64-url-encoded message via `message:<BASE64-URL ENCODED TEXT>`, which will be shown to the end user in the MitID app in some cases.
For example, if you want to show a "Transfer EUR 1000 to us" message to the user, add a `message:VHJhbnNmZXIgRVVSIDEwMDAgdG8gdXM=` value to the `login_hint`.