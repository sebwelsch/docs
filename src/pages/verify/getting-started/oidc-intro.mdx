...
// Javascript
'Authorization': "Basic " + btoa(`${encodeURIComponent(CRIIPTO_CLIENT_ID)}:${CRIIPTO_SECRET}`)
```

```csharp
// C#
"Basic " + Convert.ToBase64String(HttpUtility.UrlEncode(CRIIPTO_CLIENT_ID) + ":" + HttpUtility.UrlEncode(CRIIPTO_SECRET))
```