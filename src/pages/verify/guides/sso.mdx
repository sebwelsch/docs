...
You enable and disable `SSO` per DNS domain through the management dashboard for Criipto Verify. Go to the `Domains` tab on [manage.criipto.id](https://manage.criipto.id/domains) and click on the domain you choose for SSO. The following options are available:

import image from './images/sso-settings-for-domain.png';

<img src={image} alt="Single Sign-On Settings" />
