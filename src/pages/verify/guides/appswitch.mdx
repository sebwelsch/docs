...
If you are building a native app, and would like to get the smoothest possible UX, you can use Idura Verify's app-switching capabilities (for eIDs that support it).
...
<Highlight icon="info">
  We offer examples projects in [Kotlin](https://github.com/criipto/criipto-verify-android-example)
  and [Swift](https://github.com/criipto/criipto-verify-ios-example), which shows you how to do app
  switching.
</Highlight>
...
After the app links are configured properly, register the app link as a _Callback URL_ in Idura, and set it as the `redirect_uri` query parameter in the `authorize request` towards Idura Verify.
