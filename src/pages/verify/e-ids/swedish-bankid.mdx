...
3. Get a BankID for test. _Log in at the <a href="https://demo.bankid.com/" target="_blank">Demo bank website</a> to create a test user. If you don't have a production BankID, select the **Personal code** option to log in. The code will be sent to you by email. Once logged in, create a test user by selecting **Mobile BankID** or **BankID on file** (for desktop version) option under **Issue BankID for test**. Please note that a test user must be generated using a valid Swedish personal identity number ("Personnummer"). After creating a test user, you will be able to link it to the mobile or desktop version of the test app. 
For future test user issuances, you can log in using an already-issued BankID test user by selecting the **Test BankID** option during login._
