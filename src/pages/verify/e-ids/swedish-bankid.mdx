...
### Optional claims for signature verification

If your use case requires verifying the signature (which is <a href="https://www.bankid.com/utvecklare/guider/teknisk-integrationsguide/verifiera-signaturer" target="_blank">not required, but possible</a>), you have an option to request the evidence data.
...
In this case, a more compact ID Token with only essential claims will be issued first. The client can then make further API calls to the server, and retrieve the complete user profile from the Userinfo endpoint.
