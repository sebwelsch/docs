...
To address this concern, consider changing the <a href="https://openid.net/specs/oauth-v2-multiple-response-types-1_0.html" target="_blank">response type</a> in your [authorize request](/verify/getting-started/oidc-intro/#example-authentication-url) to `response_type=code id_token`.

In this case, a more compact ID Token with only essential claims will be issued first from the Token endpoint. The client can subsequently fetch all data about the user from the Userinfo endpoint.
