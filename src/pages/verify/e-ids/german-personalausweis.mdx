...
It can be configured to simulate a physical card for testing purposes ([instructions](https://www.ausweisapp.bund.de/fileadmin/user_upload/AusweisApp-2.2.2-NetInstallation_Integration.pdf)).
On mobile, choose "SIM" when prompted to present a card and continue.
The simulator always uses the same hard-coded test user.
...
For applications configured to use a `static` `scope` strategy, the profile will always be `Pseudonym`.
