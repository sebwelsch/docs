...
MitID supports a "ReAuthentication" flow where the username input step can be skipped if you know the users cpr number or uuid ahead of time.

`login_hint=sub:<cpr>` to trigger re-authentication when CPR number is known.
