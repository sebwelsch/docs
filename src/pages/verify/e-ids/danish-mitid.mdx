...
MitID via Criipto currently offers two levels of assurance via the acr_values `urn:grn:authn:dk:mitid:low` + `urn:grn:authn:dk:mitid:substantial`.
...
`urn:grn:authn:dk:mitid:substantial` allows users to login with username + a secondary authentication mechanism, like the MitID app or a MitID code display.

### Low

`urn:grn:authn:dk:mitid:low` allows users to login with username + password and any `substantial` authenticator.
